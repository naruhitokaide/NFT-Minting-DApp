...
export default function HomePage() {
  ...
  const mintNFT = async () => {
    if (!isConnected) {
      ...
      const cost = ethers.utils.parseEther(
        (publicSalePrice * amount).toString()
      );
      ...
      if (balance_i >= cost_i) {
        ...
        try {
          ...
          response.wait().then(async res => {
            ...
            const _nftHolds = parseInt(
              await contract.balanceOf(wallet.accounts[0].address)
            );
        ...
        toast.warn(
          ...
          ethers.utils.parseEther((publicSalePrice * amount).toString())
        );
        return;
      }
    }
      ...
      <Container>
        <Row className="my-5 py-5 justify-content-center aligin-items-center">
          ...
          <Col
            sm={12}
            md={6}
            className="mb-2 d-flex flex-column justify-content-center aligin-items-center"
          >
            <div className="text-white text-center fs-2 mb-3 title-fs">
              Total Prize Pool:{' '}
              <span className="text-pop title-fs fs-1">
                {totalMintNumber * publicSalePrice}
              </span>{' '}
              MATIC
            </div>
            <div className="text-white text-center fs-3 mb-3 title-fs">
              Mint Price:{' '}
              <span className="text-pop title-fs fs-1">{publicSalePrice}</span>{' '}
              MATIC
            </div>
            <div className="text-white text-center fs-3 mb-3 title-fs">
              You have:{' '}
              <span className="text-pop title-fs fs-1">{nftHolds}</span> NFTs
            </div>
            ...
            <Col sm={10} md={6} className="mx-auto">
              <InputGroup>
                <Button
                  ...
                  id="button-addon1"
                  className="fs-1 px-4 text-white title-fs border-1 border-white"
                ...
                <Button
                  variant="outline-secondary"
                  id="button-addon1"
                  className="fs-1 title-fs px-4 text-white border-1 border-white"
            ...
            {isConnected ? (
              <Row className="mt-3">
                ...
                <Col sm={12} md={6} className="mb-2 mx-auto">
                  <Button
                    className="mint-button py-2 px-4 title-fs rounded-5 border-white border-2 w-100"
                    onClick={mintNFT}
                  >
                    Mint Now
                  </Button>
